...
export default async function DashboardBlockPage() {
  const [deployments, activity] = await Promise.all([getDeployments(), getActivity()]);

  return (
    <>
      <Topbar crumbs={["atlas-web", "Dashboard"]} />
      <Page>
        <PageHead
          kicker="Project · atlas-web"
          title="Dashboard"
          sub="Overview of builds, pipeline state, and recent activity — the default landing surface from the Orbit IDP reference."
          actions={
            <>
              <Button size="sm">
