...
type OpsRow = [ts: string, kind: string, svc: string, ref: string, target: string, state: string, dur: string];
...
const stateTone: Record<string, "ok" | "info" | "warn" | "err"> = {
  READY: "ok", BUILD: "info", QUEUE: "warn", DONE: "ok", ROTATED: "info", FAILED: "err",
};
...
const opsColumns: readonly Column<OpsRow>[] = [
  ...
  { key: "tgt",  header: "sha / target", cell: (r) => <span className="text-fg-muted">{r[4]}</span> },
  { key: "st",   header: "state", cell: (r) => <Badge tone={stateTone[r[5]] ?? "neutral"} dot>{r[5]}</Badge> },
  { key: "dur",  header: "dur",   align: "right", cell: (r) => <span className="text-fg-muted">{r[6]}</span> },
  { key: "chev", header: "",      align: "right", width: 40, cell: () => <span className="text-fg-faint">→</span> },
];

const routeColumns: readonly Column<(typeof routeRows)[number]>[] = [
  { key: "method", header: "", width: 60, cell: (r) => <span className="text-accent">{r[0]}</span> },
  { key: "path",   header: "", cell: (r) => <span className="font-medium">{r[1]}</span> },
  { key: "lat",    header: "", align: "right", cell: (r) => <span className="text-fg-muted">{r[2]}</span> },
  { key: "reg",    header: "", align: "right", width: 50, cell: (r) => <span className="text-fg-subtle">{r[3]}</span> },
];
      ...
      <PageHead
        ...
        actions={
          <>
            ...
            <Button size="sm" variant="primary">[n] new deployment</Button>
          </>
        }
      />
